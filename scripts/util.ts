...
export async function swapExactAVAXForTokens(tokenAddress: string, router: Contract, user: SignerWithAddress, _value: any) {
    await router.connect(user).swapExactAVAXForTokens(
        0, //amountOutMin
        [chains[default_chain]?.wChainCoin, tokenAddress], //path
        user.address,
        2648069985, // Saturday, 29 November 2053 22:59:45
...
export async function transferEth(from: SignerWithAddress, _to: string, amount: string | number) {
    const tx = {
        to: _to,
