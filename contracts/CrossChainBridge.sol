...
import {IMultiRouter} from "./IMultiRouter.sol";

/// @title - A simple messenger contract for transferring/receiving tokens and data across chains.
/// @dev - This example shows how to recover tokens in case of revert
contract CrossChainBridge is CCIPReceiver, OwnerIsCreator, ReentrancyGuard {
    ...
    // The router contract address for perform swap.
    IMultiRouter public multiRouter;

    address public devAddress;

    uint256 public devFee = 100; // 1%
    uint256 public gasFee = 100; // 1%
    uint256 public chainId = 43113; //block.chainid;
    ...
    function setDevFee(uint256 _devFee) external onlyOwner {
        if (_devFee + gasFee > 1000) revert InvalidFee(_devFee);
        devFee = _devFee;
    }

    function setGasFee(uint256 _gasFee) external onlyOwner {
        if (_gasFee + devFee > 1000) revert InvalidFee(_gasFee);
        gasFee = _gasFee;
    }
        ...
        returns (bytes32 messageId)
    {
        ...
        multiRouter.swapNativeForTokens{value: amount}(
            chainId,
            usdcAddress,
            address(this),
            amount
        ...
        // transfer fee
        (bool sentDev, ) = payable(devAddress).call{
            value: (msg.value * gasFee) / 1000
        }("");
        if (!sentDev)
            revert FailedToWithdrawEth(msg.sender, devAddress, amount);
